...
export default {
  ...
  data() {
    ...
    //   device:{},
      ...
      perPage: 10,
      fields: [
        { name: 'license_key', title: '授权码', sortField: 'license_key' },
        { name: 'quantity', title: '授权数量', sortField: 'quantity' },
        { name: 'device_license_start_date', title: '激活日期', sortField: 'device_license_start_date' },
        { name: 'quantity_used', title: '已激活设备', sortField: 'quantity_used' },
        { name: '__slot:actions', title: '激活与分配设备' },
      ],
      css: {
        pagination: {
  ...
  mounted() {
    this.data = this.licensesList
  },
  methods:{
      init(){
        this.licensesList = []
        this.$http.get('/api/Licenses/AllList')
            .then((response) => {
              ...
              licensesData.quantity = item.quantity
              licensesData.quantity_used = item.quantity_used
              licensesData.device_license_start_date = item.device_license_start_date
     ...
     */
    onPaginationData(licensesList) {
      this.$refs.pagination.setPaginationData(licensesList)
      this.$refs.paginationInfo.setPaginationData(licensesList)
    },
              ...
              >
                <template slot="actions" >
                  <div class="table-button-container">
                    <b-button variant="outline-success" class="ml-2" href="/licenses/assign">
                        <i class="uil-cloud-computing"></i> 激活与分配设备
                    </b-button>
                  </div>
                </template>
